  ...
  def menu
    ...
    while input != "exit"
      ...
      if input.to_i > 0 
        ...
        puts "#{the_shoe.name.strip.colorize(:white).bold} - #{the_shoe.price.colorize(:yellow).underline} - #{the_shoe.availability.colorize(:green)} - #{the_shoe.description.colorize(:light_blue)} -> #{the_shoe.url.colorize(:green).underline}"
        elsif input == "list"
          list_shoes 
      end
    end 
